<?php
class TCConditionMise_View 
	{
	private $model;
	private $controller;
	private $mysqli;
	private $baseURL = 'index.php?model=TCConditionMise';
	
	public final function __construct(TCConditionMise $_model, TCConditionMise_Controller $_controller) 
		{
		$this->model = $_model;
	...
	public final function AfficherToutesTechniques()
		...
		if($query->num_rows == 0)
			$contenuPrincipal = '<center>Il n\'y a pas de technique de mise.</center>';
		else
			{
			$contenuPrincipal = '
			<table>
			...
			while($data = $query->fetch_assoc())
				...
				<tr>
					...
					<td>TCMise_'.$data['id'].'.class.php</td>
					<td>
		...
		$moreInfo[0]['contenu'] = '<a href="'.$this->baseURL.'&action=FormulaireAjouterTechnique">Ajouter une technique de mise</a>';
	...
	public final function SupprimerTechnique()
		{
		$req = "SELECT * FROM tcconditionmise WHERE id=".$_GET['id'];
	...
	public final function FormulaireAjouterTechnique()
		...
		$titrePage = "Ajouter une technique de mise";
		$contenuPrincipal = '
		<ul>
			'.$error.'
		</ul>
	...
	public final function FormulaireEditerTechnique()
		...
		$titrePage = "Détail de la technique de mise : ".$this->model->RechercherTitre();
		$contenuPrincipal = '
		<ul>
			'.$error.'
		</ul>
	...
	public final function AfficherInfosTechnique()
		...
		<table>
			...
			<tr>
				...
				<td>
					<span>TCMise_'.$data['id'].'.class.php</span>
				</td>
			</tr>
			<tr>
				<td>Description de la technique : </td>
