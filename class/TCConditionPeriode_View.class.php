<?php
class TCConditionPeriode_View 
	{
	private $model;
	private $controller;
	private $mysqli;
	private $baseURL = 'index.php?model=TCConditionPeriode';
	
	public final function __construct(TCConditionPeriode $_model, TCConditionPeriode_Controller $_controller) 
		{
		$this->model = $_model;
	...
	public final function AfficherToutesTechniques()
		...
		if($query->num_rows == 0)
			...
			while($data = $query->fetch_assoc())
				...
				<tr>
					...
					<td>TCPeriode_'.$data['id'].'.class.php</td>
					<td>
		...
		$moreInfo[0]['contenu'] = '<a href="'.$this->baseURL.'&action=FormulaireAjouterTechnique">Ajouter une technique de période</a>';
	...
	public final function SupprimerTechnique()
		{
		$req = "SELECT * FROM tcconditionperiode WHERE id=".$_GET['id'];
	...
	public final function FormulaireAjouterTechnique()
		...
		$titrePage = "Ajouter une technique de période";
		$contenuPrincipal = '
		<ul>
			'.$error.'
		</ul>
	...
	public final function AfficherInfosTechnique()
		...
		<table>
			...
			<tr>
				...
				<td>
					<span>TCPeriode_'.$data['id'].'.class.php</span>
				</td>
			</tr>
			<tr>
				<td>Description de la technique : </td>
